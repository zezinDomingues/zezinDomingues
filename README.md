<!-- ======================= BANNER ======================= -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:4285F4,50:1572B6,100:777BB4&height=220&section=header&text=José%20Domingues&fontColor=ffffff&fontSize=48&fontAlignY=38&desc=Desenvolvedor%20Back-end%20•%20Cloud%20•%20Data%20Science&descAlignY=58&descSize=18&animation=fadeIn" alt="banner"/>

<!-- ======================= TYPING ======================= -->
<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=24&pause=1000&color=1572B6&center=true&vCenter=true&width=650&lines=Desenvolvedor+Back-end+J%C3%BAnior;PHP+%7C+Python+%7C+SQL;Cloud+com+AWS+%26+Google+Cloud;Cursando+Ci%C3%AAncia+de+Dados+na+FIAP)](https://git.io/typing-svg)

<!-- ======================= SOCIALS ======================= -->
<a href="https://www.linkedin.com/in/jos%C3%A9-domingues1/"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/></a>
<a href="mailto:contato@josedomingues.digital"><img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"/></a>
<a href="https://github.com/zezinDomingues"><img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/></a>
<img src="https://komarev.com/ghpvc/?username=zezinDomingues&label=Perfil&color=1572b6&style=for-the-badge" alt="views"/>

</div>

<br/>

<!-- ======================= SOBRE ======================= -->
## 🚀 Sobre mim

```yaml
nome:        José Otilio Domingues dos Santos
cargo:       Desenvolvedor Back-end Júnior @ DOTSEG
localização: São Paulo, Brasil 🇧🇷
foco:        Visão Computacional • Monitoramento em tempo real • Cloud
estudando:   Ciência de Dados @ FIAP
lema:        "Infraestrutura robusta, escalável e segura é o pilar da inovação"
```

- 🔭 Desenvolvendo soluções de **visão computacional** e monitoramento inteligente de equinos na **DOTSEG**
- 🌱 Unindo desenvolvimento **back-end** à **análise avançada de dados**
- ☁️ Certificado em **Cloud Computing** pela **AWS** e **Google Cloud**
- 🛡️ Experiência sólida em **segurança, performance** e grandes portais, SaaS e sistemas personalizados

<br/>

<!-- ======================= STACK ======================= -->
## 🛠️ Tech Stack

<div align="center">

### Linguagens & Back-end
<img src="https://skillicons.dev/icons?i=php,python,mysql,sqlite,html,css,js&theme=dark" alt="stack"/>

### Cloud, Ferramentas & CMS
<img src="https://skillicons.dev/icons?i=aws,gcp,git,github,linux,wordpress,vscode&theme=dark" alt="tools"/>

</div>

<br/>

<!-- ======================= EXPERIÊNCIA ======================= -->
## 💼 Experiência

> ### 🐴 DOTSEG — Desenvolvedor Júnior · `jul/2026 – atual`
> Soluções de **visão computacional** para bem-estar animal: monitoramento inteligente com rastreamento em tempo real de equinos (postura e localização), análise de sinais vitais para detecção precoce de cólicas e módulos de presença humana.

> ### 🌐 2WP — Desenvolvedor Web Júnior · `ago/2025 – jul/2026`
> Sites, portais de notícias, SaaS e softwares web personalizados. Ciclo completo de projetos, otimização de **performance e segurança** (remediação de vulnerabilidades) com **PHP, MySQL, HTML e CSS**.

<br/>

<!-- ======================= FORMAÇÃO ======================= -->
## 🎓 Formação & Certificações

<table>
<tr>
<td width="50%" valign="top">

**🏫 Formação**
- **FIAP** — CST em Ciência de Dados `2026–2027`
- **SENAI SP** — Análise e Desenvolvimento de Sistemas `2024–2025`

</td>
<td width="50%" valign="top">

**📜 Certificações**
- Python Essentials 1
- Google Cloud Computing Foundations
- AWS Academy — Cloud Foundations
- App Dev Environment on GCP (Skill Badge)
- Algoritmos: Aprenda a Programar

</td>
</tr>
</table>

<br/>

<!-- ======================= STATS ======================= -->
## 📊 GitHub Analytics

<div align="center">

<img height="165em" src="https://github-readme-stats.vercel.app/api?username=zezinDomingues&show_icons=true&theme=react&hide_border=true&bg_color=0D1117&include_all_commits=true&count_private=true" alt="stats"/>
<img height="165em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=zezinDomingues&layout=compact&theme=react&hide_border=true&bg_color=0D1117&langs_count=8" alt="langs"/>

<img width="95%" src="https://github-readme-streak-stats.herokuapp.com/?user=zezinDomingues&theme=react&hide_border=true&background=0D1117" alt="streak"/>

<img src="https://github-profile-trophy.vercel.app/?username=zezinDomingues&theme=onedark&no-frame=true&no-bg=true&margin-w=4&row=1" alt="trophies"/>

<img width="95%" src="https://github-readme-activity-graph.vercel.app/graph?username=zezinDomingues&theme=react-dark&hide_border=true&bg_color=0D1117&color=1572B6&line=4285F4&point=ffffff" alt="activity"/>

</div>

<!-- ======================= FOOTER ======================= -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:777BB4,50:1572B6,100:4285F4&height=120&section=footer" alt="footer"/>
